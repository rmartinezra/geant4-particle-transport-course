...
void SteppingAction::UserSteppingAction(const G4Step* aStep)
{
 ...
 Run* run 
  ...
  // One row for every primary: an interaction distance or an explicit
  // right-censoring distance at escape.  No escaped event is discarded.
  const G4StepPoint* prePoint = aStep->GetPreStepPoint();
  const G4Material* material = prePoint->GetMaterial();
  G4String isotopeName = "unknown";
  if (material->GetNumberOfElements() == 1) {
    const G4Element* element = material->GetElement(0);
    if (element->GetNumberOfIsotopes() > 0) {
      isotopeName = element->GetIsotope(0)->GetName();
    }
  }
  const G4Event* event = G4RunManager::GetRunManager()->GetCurrentEvent();
  G4AnalysisManager* courseAnalysis = G4AnalysisManager::Instance();
  courseAnalysis->FillNtupleIColumn(0, event->GetEventID());
  courseAnalysis->FillNtupleSColumn(1, aStep->GetTrack()->GetDefinition()->GetParticleName());
  courseAnalysis->FillNtupleDColumn(2, prePoint->GetKineticEnergy()/eV);
  courseAnalysis->FillNtupleSColumn(3, material->GetName());
  courseAnalysis->FillNtupleSColumn(4, isotopeName);
  courseAnalysis->FillNtupleDColumn(5, material->GetDensity()/(g/cm3));
  courseAnalysis->FillNtupleDColumn(6, material->GetTotNbOfAtomsPerVolume()*cm3);
  courseAnalysis->FillNtupleDColumn(7, aStep->GetStepLength()/cm);
  courseAnalysis->FillNtupleIColumn(8, transmit ? 0 : 1);
  courseAnalysis->FillNtupleIColumn(9, transmit ? 1 : 0);
  courseAnalysis->FillNtupleSColumn(10, process ? process->GetProcessName() : "none");
  courseAnalysis->AddNtupleRow();
