...
void Run::EndOfRun()
{
  ...
  // Machine-readable record for the course scan. The reconstructed
  // attenuation uses only the Monte Carlo counts. The calculator value is
  // emitted separately and used only for posterior validation.
  G4int interacted = totalCount - survive;
  G4double referenceMu = emCalculator.ComputeCrossSectionPerVolume(
      fEkin, fParticle, "compt", material);
  G4double referenceSigma = referenceMu/atomDensity;
  G4double atomicNumber = 0.;
  if (material->GetNumberOfElements() == 1) {
    atomicNumber = (*material->GetElementVector())[0]->GetZ();
  }
  G4cout << std::setprecision(12)
         << "COURSE_RESULT_A"
         << ",thickness_cm=" << tickness/cm
         << ",N0=" << numberOfEvent
         << ",N_transmitted=" << survive
         << ",N_interacted=" << interacted
         << ",material=" << material->GetName()
         << ",density_g_cm3=" << density/(g/cm3)
         << ",atomic_number=" << atomicNumber
         << ",atomic_number_density_cm3=" << atomDensity*cm3
         << ",energy_keV=" << fEkin/keV
         << ",geant4_compton_mu_cm_inv=" << referenceMu*cm
         << ",geant4_compton_sigma_cm2_atom=" << referenceSigma/cm2
         << ",processes=Transportation|compt"
