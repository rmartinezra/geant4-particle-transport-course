...
void HistoManager::Book()
{
  ...
  // One row per incident primary.  The original histograms are preserved;
  // this ntuple adds the event-wise correlations needed by the course.
  analysisManager->CreateNtuple("mcs", "Outgoing primary direction");
  analysisManager->CreateNtupleIColumn("event_id");
  analysisManager->CreateNtupleDColumn("energy_GeV");
  analysisManager->CreateNtupleDColumn("momentum_GeV_c");
  analysisManager->CreateNtupleDColumn("thickness_cm");
  analysisManager->CreateNtupleDColumn("theta_x_rad");
  analysisManager->CreateNtupleDColumn("theta_y_rad");
  analysisManager->CreateNtupleDColumn("theta_total_rad");
  analysisManager->CreateNtupleIColumn("transmitted");
  analysisManager->CreateNtupleDColumn("final_energy_GeV");
  analysisManager->CreateNtupleDColumn("density_g_cm3");
  analysisManager->CreateNtupleDColumn("radiation_length_cm");
  ...
  analysisManager->SetNtupleFileName(0, "mcs_events.csv");
}
