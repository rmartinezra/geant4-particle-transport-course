...
void TrackingAction::PreUserTrackingAction(const G4Track* aTrack )
{
  ...
  if (aTrack->GetTrackID() == 1) {
    ...
    fInitialDirection = aTrack->GetMomentumDirection().unit();
    fInitialEnergy = aTrack->GetKineticEnergy();
...
void TrackingAction::PostUserTrackingAction(const G4Track* aTrack)
{
  ...
  // Event-wise primary result.  theta_total uses the dot product and is
  // therefore valid for any incident direction.  The projected basis is
  // constructed so that for TestEm5's +X beam it is exactly (+Y,+Z).
  if (aTrack->GetTrackID() == 1) {
    const G4ThreeVector finalDirection = direction.unit();
    G4ThreeVector transverseX = G4ThreeVector(0., 1., 0.)
      - fInitialDirection.y()*fInitialDirection;
    if (transverseX.mag2() < 1.e-20) {
      transverseX = G4ThreeVector(1., 0., 0.)
        - fInitialDirection.x()*fInitialDirection;
    }
    transverseX = transverseX.unit();
    const G4ThreeVector transverseY = fInitialDirection.cross(transverseX).unit();
    const G4double longitudinal = finalDirection.dot(fInitialDirection);
    G4double cosine = longitudinal;
    cosine = std::max(-1.0, std::min(1.0, cosine));
    const G4double thetaX = std::atan2(finalDirection.dot(transverseX), longitudinal);
    const G4double thetaY = std::atan2(finalDirection.dot(transverseY), longitudinal);
    const G4double thetaTotal = std::acos(cosine);
    const G4Event* event = G4RunManager::GetRunManager()->GetCurrentEvent();
    const G4Material* material = fDetector->GetAbsorberMaterial();
    analysisManager->FillNtupleIColumn(0, event->GetEventID());
    analysisManager->FillNtupleDColumn(1, fInitialEnergy/GeV);
    analysisManager->FillNtupleDColumn(2, fInitialMomentum/GeV);
    analysisManager->FillNtupleDColumn(3, fDetector->GetAbsorberThickness()/cm);
    analysisManager->FillNtupleDColumn(4, thetaX);
    analysisManager->FillNtupleDColumn(5, thetaY);
    analysisManager->FillNtupleDColumn(6, thetaTotal);
    analysisManager->FillNtupleIColumn(7, transmit ? 1 : 0);
    analysisManager->FillNtupleDColumn(8, energy/GeV);
    analysisManager->FillNtupleDColumn(9, material->GetDensity()/(g/cm3));
    analysisManager->FillNtupleDColumn(10, material->GetRadlen()/cm);
    analysisManager->FillNtupleDColumn(11, fInitialDirection.x());
    analysisManager->FillNtupleDColumn(12, fInitialDirection.y());
    analysisManager->FillNtupleDColumn(13, fInitialDirection.z());
    analysisManager->FillNtupleDColumn(14, finalDirection.x());
    analysisManager->FillNtupleDColumn(15, finalDirection.y());
    analysisManager->FillNtupleDColumn(16, finalDirection.z());
