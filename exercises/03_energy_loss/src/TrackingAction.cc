...
: fRunAction(runaction), fEventAction(eventaction), fDetector(detector)
{ 
  fEkin1 = 0.;
}
...
void TrackingAction::PostUserTrackingAction(const G4Track* track)
{
  ...
  if (track->GetTrackID() == 1) {
    ...
    const G4bool transmitted = Ekin2 > 0. &&
      track->GetPosition().x() >= 0.499999*fDetector->GetSize();
    fEventAction->SetPrimaryResult(fEkin1, Ekin2, transmitted);
